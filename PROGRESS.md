...
- **Strict Cleaning Pass**: Executed an ultra-strict dictionary and pattern cleaning pass on the merged corpus as requested by the user. Stripped `</s>` tokens, removed `chemunhu:` morphological artifacts, discarded lines <15 chars, and deduplicated. Most critically, applied the strict 25% non-dictionary filter against the 12,850 base wordlist in `shona_words.txt`. Due to Shona's highly agglutinative grammar (where the uninflected dictionary lacks conjugated prefixes/proclitics), this filter reduced the corpus from **252,071 lines (3,912,233 tokens)** to **81 lines (484 tokens)**. Reported stats and samples to the user before retraining.
